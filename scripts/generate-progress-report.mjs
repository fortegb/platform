...
<head>
  ...
  <style>
    :root {
      ...
      --border: #e2e8ef;
      --radius: 12px;
      --shadow: 0 4px 24px rgba(32, 48, 69, 0.08);
    }
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .toolbar a { background: var(--blue); color: #fff; }
    .toolbar a.secondary { background: transparent; color: var(--navy); border: 1px solid var(--border); }
    .toolbar button { background: var(--navy); color: #fff; border: none; cursor: pointer; }
    .toolbar button:hover { background: var(--blue); }
    .hero {
      background: linear-gradient(135deg, var(--navy) 0%, #2a4560 50%, var(--slate) 100%);
    ...
    h2 { font-size: 1.3rem; font-weight: 700; color: var(--navy); }
    .card {
      background: var(--card); border-radius: var(--radius); box-shadow: var(--shadow);
      padding: 1.5rem; margin-bottom: 1rem; border: 1px solid var(--border);
    }
    .card h3 { font-size: 1rem; font-weight: 600; color: var(--navy); margin-bottom: 0.75rem; }
    ...
    .progress-wrap {
      height: 4px; background: var(--bg); position: relative; margin: 0 1.25rem;
    }
...
<body>
  ...
  <main class="container">
    <section id="agora">
      ...
      ${focusNotes ? `<div class="card"><h3>Notas para sócios</h3>${sectionBodyToHtml(focusNotes)}</div>` : ''}
    </section>
