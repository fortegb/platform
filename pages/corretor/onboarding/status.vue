    ...
    <div class="rounded-lg border border-base-300 bg-base-100 p-8 text-center">
      ...
      <div class="mt-8 flex flex-col sm:flex-row items-center justify-center gap-3">
        <NuxtLink
          ...
          class="w-full sm:w-auto inline-flex items-center justify-center border border-transparent bg-secondary text-white hover:bg-primary-500 px-5 py-2.5 rounded-lg text-sm font-semibold transition-colors"
        >
          Ir para o painel
        </NuxtLink>
        ...
        <a
          ...
          target="_blank"
          rel="noopener"
          class="w-full sm:w-auto inline-flex items-center justify-center gap-2 border border-transparent bg-whatsapp hover:bg-whatsapp-hover text-white px-5 py-2.5 rounded-lg text-sm font-semibold transition-colors"
        >
        ...
        <NuxtLink
          to="/"
          class="w-full sm:w-auto inline-flex items-center justify-center border border-primary-500 text-primary-500 hover:bg-primary-500 hover:text-white px-5 py-2.5 rounded-lg text-sm font-semibold transition-colors"
        >
          Voltar ao início
        </NuxtLink>
      </div>
...
const TONES: Record<Variant, {
  ...
  pending: {
    ...
    message:
      'A ForteGB revisa seu cadastro em pouco tempo e avisa você por e-mail e WhatsApp assim que sair o resultado. Você pode fechar esta página — nada se perde.',
    label: 'text-warning',
