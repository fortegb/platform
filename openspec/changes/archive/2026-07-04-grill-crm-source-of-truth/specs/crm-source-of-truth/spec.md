# CRM source of truth & client model
...
The system SHALL record client interest as a `registro` per (cliente × casa) carrying status, broker attribution (null = direct), and registration timestamp, with an append-only audit history of commission-relevant events.

#### Scenario: Same person, different houses
- **WHEN** the same client is broker-attributed on one house and direct on another
- **THEN** each house has its own `registro` with independent status and attribution

#### Scenario: Audit is viewable in the UI
- **WHEN** staff or the attributed broker opens a client/house record
- **THEN** the registration, attribution, exceptions, and status changes are visible in the UI without running database queries
