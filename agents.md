...
> **A espinha do projeto é o [Roteiro](./docs/planning/roteiro.md)** — 9 passos em 3 estágios (Definição 1–7 · Execução 8 · Evolução 9), com gates G1/G2/G3. É **controlador**, não decorativo, e é representado no board (campo `Etapa` + `Milestone` + tipo nativo), não em prosa. **Passo 4 Arquitetura fechado** — infra/ambientes (#146) e arquitetura de domínio (#179: visitas, mensageria, RBAC, admin) ambos fechados. **Passo 5 Jornadas fechado** — epic #176, todas as 11 leaves concluídas (#185–#195, D-057..D-067), incluindo a lacuna de CPF do corretor (#196, D-068). **Passo atual: 6 Design system**. Ver `roteiro.md` antes de escolher trabalho.
...
### Estágio atual
- **Passo 4 (Arquitetura) — fechado.** A1–A4 + B1–B4 + C1–C3 + D1–D4 + E1–E4 + CI/CD (#166–#169) + Dev local (#170–#172) → Epic #146 fechado (D-025..D-051). Arquitetura de domínio → Epic #179 fechado, 5 leaves: Tuya (#181, D-052), visitas/identidade (#180, D-053), mensageria (#182, D-054 — corrige "Telegram-first" para escopo interno-only, WhatsApp sempre externo), RBAC (#183, D-055), admin (#184, D-056, emenda a `platform-architecture`).
- **Passo 5 (Jornadas) — fechado.** Epic **#176**, todas as 11 leaves concluídas (#185–#195): descoberta do site (#185, D-057), visita agendada (#186, D-058), visita instantânea QR (#187, D-059 — reabre D-053 para renovação limitada por telefone), pós-visita/reengajamento (#188, D-061 — greenfield, magic link + `revoke()`), onboarding corretor (#189, D-062 — reabre `crm-source-of-truth` para a amarra `corretor_casa`), registro cliente/comissão (#190, D-063 — corrige condição de corrida real), pipeline corretor (#191, D-064 — `registro.status` definido pela 1ª vez), fila de exceção de verificação (#192, D-060 — 1ª leaf greenfield), operação diária staff (#193, D-065 — puramente consumidora), gestão de acesso Tuya (#194, D-066 — re-confirmação, sem build), config de plataforma/papéis (#195, D-067 — corrige rotas `/admin/*`→`/staff/*` e chaves API vs. D-043). Lacuna encontrada durante #190 virou issue separada **#196** (CPF do corretor, reabre D-062) — **fechada** (D-068): CPF obrigatório no perfil junto com WhatsApp. **G2** segue gated até 6 (Design system) + 7 (Versionamento). **Passo atual: 6 Design system.** Ver **`roteiro.md`** + **`STATUS.md`**.
- **Passos 1–2 validados (D-028 / #177):** contexto + mapa de funcionalidades aceitos.
- **Grillings concluídos:** #145 (fundacional), #28 (CRM), #33 (home), roteiro (#173), passos 1–2 (#177), A–E config + lifecycle contract, Tuya/visitas/mensageria/RBAC/admin (#179), 11 jornadas de Passo 5 (#176) → **D-015..D-067**. Stack confirmada: serverless Vercel, **Sanity** + **Supabase master** + HubSpot sync, WhatsApp externo/Telegram interno-only (D-054). MVP em **v1/v2/v3** (D-018).
- **Decisões técnicas:** fechadas em `decisions.md` (D-015..D-067); o que resta é `deferred` (tours v2, media v3, mobile — Etapa 8 agora, #29–31/#140) — reabre no grilling da fase.
