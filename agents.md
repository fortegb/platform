...
- **Grillings concluídos:** #145 (fundacional), #28 (CRM), #33 (home), roteiro (#173), passos 1–2 (#177), A1–A4 + B1–B4 + C1–C3 CMS → **D-015..D-036**. Stack confirmada: serverless Vercel, **Sanity** + **Supabase master** + HubSpot sync, Telegram-first. MVP em **v1/v2/v3** (D-018).
- **Decisões técnicas:** fechadas em `decisions.md` (D-015..D-036); o que resta é `deferred` (tours v2, media v3, mobile) — reabre no grilling da fase.
