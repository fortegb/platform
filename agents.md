...
### Estágio atual
- **Passo 4 (Arquitetura) — em curso.** Definição (produto/stack, passos 1–3) concluída; A1 ambientes (local/staging/prod) especificado (D-025, página Ambientes); faltam as restantes folhas de #146 (#148–172), depois 5 (jornadas re-validação) e 6 (design). **G2: build da Fase 1 (#48/#56) gated até 4–6 fecharem** (`v0 Definição` a 100%). **Próximo:** #148 mapeamento branch→ambiente. Ver **`roteiro.md`** + **`STATUS.md`**.
