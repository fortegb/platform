...
### D-061 — Jornada: pós-visita e reengajamento (2026-07-12) — **#188**

- **Status:** accepted
- **Contexto:** quinta leaf de Passo 5 (Jornadas, epic #176) grillada. As duas jornadas de visita (#186, #187) terminam em "senha entregue" — nada existe depois: sem lembrete, sem cancelamento/reagendamento, sem follow-up. `screen-map.md` já sinalizava essa lacuna ("Follow-up pós-visita → #141 · sem tela MVP") e #141 (Execução) lista canal/timing/consentimento como questões em aberto próprias. Leaf totalmente greenfield — nenhum código existia para nenhum dos três sub-fluxos.
- **Decisão:**
  - **Lembrete pré-visita:** ~24h antes, transacional (D-054 já nomeia "lembretes" explicitamente) — sem questão de consentimento nova.
  - **Cancelamento/reagendamento self-service via magic link (escolhido sobre WhatsApp-mediado durante exploração):** considerada a opção de o visitante contatar staff via WhatsApp (reaproveitando o padrão de escape hatch de #186/#187/#192, zero UI nova) mas rejeitada explicitamente em favor de self-service — o visitante resolve sozinho, sem ida-e-volta, e zero tempo de staff é gasto por cancelamento, por mais raro que seja. Link de token único, alta entropia, sem cronômetro de expiração próprio — válido até a visita atingir estado terminal (`completed`, `cancelled`, `declined`) ou o horário agendado passar.
  - **Link entregue nas mensagens já existentes, sem envio novo:** anexado à confirmação de agendamento (#186) e repetido no lembrete pré-visita (novo nesta leaf) — sem tipo de mensagem adicional.
  - **Novo status terminal `cancelled`, aditivo, não uma modificação de `visit-identity-verification`:** essa capability é escopada a mecanismo/gating de verificação, nunca reivindicou enumerar todos os status terminais. Cogitou-se dobrar cancelamento dentro de `declined` — rejeitado: `declined` hoje significa "falha de verificação" (sinal de segurança); misturar com "visitante mudou de ideia" tornaria esse sinal ruidoso sem benefício.
  - **Cancelar após `access_provisioned` chama `revoke`:** D-052 já nomeava `revoke(credential)` na interface do adapter, mas nada a chamava — um código de porta ativo nunca deveria sobreviver a uma visita explicitamente cancelada. Primeiro caller concreto dessa função, não uma mudança ao contrato do adapter.
  - **Reagendar = cancelar + reagendar pelo fluxo normal, não editar in-place:** botão "Remarcar" cancela a visita atual (mesmo caminho acima) e entra no formulário de agendamento padrão (#186), pré-preenchido — reuso de mecanismo já existente em vez de um segundo caminho "editar visita". Reuso de verificação (janela de 12 meses, ou o gate OTP do fluxo instantâneo) aplica normalmente, sem caso especial.
  - **Alerta a staff via Telegram em cancelamento/reagendamento:** mesmo padrão de notificação interna de #192 — sem parte externa envolvida, cai do lado Telegram de D-054 já fechado.
  - **Follow-up pós-visita classificado por timing, resolvendo a questão de consentimento de #141:** mesmo dia / +24h ("como foi sua visita?") = transacional, sem opt-in extra. +3 dias ou conteúdo promocional (imóveis similares, "ainda interessado?") = marketing, opt-in explícito obrigatório, off por padrão — aplica o framework já existente de D-054, não cria um novo. **Escopo desta leaf é só a regra de consentimento** — não desenha nem constrói a sequência de nutrição em si (conteúdo, cadência, ferramenta) — isso fica para Execução (#141) ou v2+ (D-018). Sem canal novo: e-mail permanece adiado (D-020, "e-mail futuro") e fora de D-054, que cobre só WhatsApp/Telegram.
  - **DoD:** docs only — D-061 + `templates/jornada-pos-visita-reengajamento.md` + nova capability OpenSpec `journey-post-visit-reengagement` + `jornadas-plataforma.md` §3.2 (substituindo o passo 10 antigo) e `screen-map.md` atualizados (nova rota `/visita/gerenciar/[token]`). Implementação real → Execução (#141, #81).
- **Rationale:** self-service prioriza zero carga operacional por cancelamento sobre menor custo de build — proporcional ao fato de que cada cancelamento evitado de contato manual composto ao longo do tempo supera o custo único de construir uma página sem autenticação. Separar `cancelled` de `declined` preserva um sinal de segurança que já existia (falha de verificação) sem diluí-lo com um evento operacional não relacionado. Resolver o consentimento de follow-up usando o framework já existente de D-054 evita inventar uma segunda política de consentimento para uma variação do mesmo problema (mensagem transacional vs. promocional).
- **Consequências:** canon fechado; #141/#81 (Execução) implementam sem reabrir arquitetura; primeiro caller real de `revoke()` estabelece o padrão para qualquer cancelamento futuro que precise revogar acesso; `jornadas-plataforma.md`/`screen-map.md` saem de rascunho para validado nesta jornada.
- **Emenda posterior:** a ordem "Remarcar cancela a visita atual e entra no formulário" foi emendada por **D-071** (#200) — o cancelamento passa a ocorrer só na confirmação do novo horário, não ao clicar Remarcar, preservando a intenção sem deixar o visitante sem visita se abandonar o formulário.

### D-062 — Jornada: onboarding do corretor (2026-07-12) — **#189**

- **Status:** accepted
- **Contexto:** sexta leaf de Passo 5 (Jornadas, epic #176) grillada. `pages/login.vue` já tem UI de autenticação identifier-first (mock), mas seu caminho de cadastro é um stub inacabado e o login checa a tabela `realtors`, anterior ao enum unificado `role` de D-055. Rascunho §4.2 (associar-se a uma casa, contrato por imóvel) nunca teve leaf própria de Passo 5 — lacuna encontrada e incorporada a esta leaf durante a exploração.
- **Decisão:**
  - **Onboarding de conta consome o modelo de `role` de D-055, não a tabela `realtors`:** cadastro cria conta com `role = corretor` e um novo campo `corretor.status` (`pending_approval | approved | rejected`) — mesmo padrão de dois estados já usado em `Cliente` (Contato→Cliente) e `visit` (pending→verified). Role sozinho não representa "cadastrado, aguardando revisão".
  - **Uma única página de gestão para staff (`/staff/corretores`), cobrindo aplicações de conta E solicitações de casa juntas:** justificada como UI custom pelo teste de D-056 (aprovação com efeito colateral + precisa ser segura para staff não-técnico), mesmo raciocínio já aplicado à fila de #192.
  - **Sem notificação push (Telegram) nesta leaf — reconsiderado e removido:** diferente da fila de #192 (visitante pode estar fisicamente esperando), nada aqui tem urgência de espera física — staff descobre pendências ao checar a página, não por ser avisado. Não é uma versão menor do padrão de #192; é a ausência corretamente escopada dele, decidida reexaminando o que de fato justificava o padrão lá.
  - **Rejeição notifica o corretor via WhatsApp:** D-054 já nomeia corretor explicitamente como parte externa coberta pela regra "sempre WhatsApp" — mesmo padrão de #192/#188.
  - **Associação por casa (§4.2, incorporada) — minuta visível antes da assinatura:** ao reclamar uma casa, o contrato não-assinado (minuta, termos específicos da casa, do template legal já existente) fica visível imediatamente tanto na página do corretor quanto no item de revisão de staff.
  - **Assinatura fica inteiramente fora da plataforma:** consistente com a decisão de MVP já fechada ("Gov.br manual-first") — esta leaf não reabre isso, só desenha a jornada ao redor. Sem mecanismo de e-signature, sem tracking de assinatura in-app.
  - **Staff faz upload do contrato assinado, não o corretor — mais seguro (staff controla o registro oficial):** reaproveita o bucket privado já existente (D-016/D-030), sem infraestrutura nova. Upload do PDF assinado **é o próprio ato de aprovação** — uma ação, não duas — `corretor_casa.status` vira `approved` na mesma chamada. Rejeição continua disponível sem exigir upload algum.
  - **Reabre `crm-source-of-truth` (mesmo tratamento que #187 deu a D-053):** o requirement "Per-house registration and audit" nunca condicionou `registro.corretor_id` a aprovação por casa — sem essa amarra, todo o mecanismo de aprovação desta leaf seria cosmético (corretor poderia registrar cliente em qualquer casa, aprovado ou não). Adiciona a restrição: `registro.corretor_id` só é válido com um `corretor_casa` `approved` para aquele par.
  - **DoD:** docs only — D-062 + `templates/jornada-onboarding-corretor.md` + nova capability OpenSpec `journey-corretor-onboarding` + delta `MODIFIED` em `crm-source-of-truth` (reabre, registrado explicitamente) + `jornadas-plataforma.md` §4.1/§4.2 e `screen-map.md` atualizados. Implementação real → Execução (#86, #50).
- **Rationale:** consumir o modelo de role de D-055 em vez do stub `realtors` evita duas fontes de verdade para "quem é corretor". Remover a notificação push depois de reexaminar por que ela existia em #192 evita copiar um padrão sem a justificativa que o originou. Staff controlar o upload do contrato assinado prioriza segurança do registro oficial sobre a conveniência marginal de self-upload, proporcional ao baixo volume do negócio. Fechar a lacuna de atribuição em `crm-source-of-truth` é o que torna a aprovação por casa real, não apenas visual — sem essa amarra, toda a leaf seria decorativa.
- **Consequências:** canon fechado; reabre `crm-source-of-truth` formalmente (registrado aqui, não absorvido em silêncio); #86/#50 (Execução) implementam sem reabrir arquitetura de novo; `jornadas-plataforma.md`/`screen-map.md` saem de rascunho para validado nesta jornada; §4.2 deixa de ser uma lacuna sem leaf própria.

### D-063 — Jornada: registro de cliente e proteção de comissão (2026-07-12) — **#190**

- **Status:** accepted
- **Contexto:** sétima leaf de Passo 5 (Jornadas, epic #176) grillada. `server/api/corretor/leads.post.ts` predata D-020 (`crm-source-of-truth`) e a amarra `corretor_casa` de #189. O stub não está só desatualizado — tem um bug real: a proteção de comissão "primeiro-registro-ganha", razão de existir desta jornada, não funciona de fato, porque o endpoint verifica duplicata via `SELECT` e insere via `INSERT` como dois passos separados (check-then-act), não uma garantia transacional única.
- **Decisão:**
  - **Modelo de dados corrigido:** registro escreve no modelo `cliente`/`registro` (D-020), não numa tabela `leads` avulsa — dedup por CPF, `registro` por cliente×casa, `historico` append-only, `fonte: portal-corretor` (valor concreto para a fonte "broker portal" já nomeada em D-020).
  - **CPF obrigatório no registro feito pelo corretor, promovendo direto a nível `Cliente`:** diferente da captura de baixo compromisso de #185 (Contato, só telefone). O trabalho do corretor aqui é justamente estabelecer um lead qualificado e identificado; D-020 já nomeia CPF como autoridade de dedup — aceitar telefone-só enfraqueceria exatamente a garantia que esta jornada existe para proteger.
  - **Corrige a condição de corrida:** a garantia "primeiro-registro-ganha" passa a ser aplicada por uma constraint de unicidade transacional no banco (`registro(cliente_id, casa_id)`), não por lógica de aplicação — a segunda submissão concorrente falha na constraint do banco, e a aplicação traduz essa falha numa resposta clara ("já registrado por você/por outro corretor"), não um erro genérico.
  - **Consome a amarra `corretor_casa` de #189:** registro só é permitido com `corretor_casa` `approved` para aquela casa — o ponto de aplicação real que #189 criou a restrição para ter.
  - **Auth corrigido:** checa `role = corretor` + `status = approved` (modelo de #189), não mais a tabela `realtors` pré-D-055.
  - **Reenvio pelo mesmo corretor é idempotente, não erro:** resubmeter mostra o status do registro já existente em vez de bloquear com mensagem de erro.
  - **Fora de escopo:** leads diretos/originados do site (#185, já fechada); reatribuição de comissão (ação de staff/admin, sem leaf própria).
  - **DoD:** docs only — D-063 + `templates/jornada-registro-cliente-comissao.md` + nova capability OpenSpec `journey-corretor-client-registration` + `jornadas-plataforma.md` §4.3 e `screen-map.md` atualizados. Implementação real → Execução (#86, #90).
- **Rationale:** o bug de corrida não é uma lacuna cosmética — é o oposto exato do que esta jornada promete ("proteção de comissão"), então corrigi-lo antes que vire comportamento em produção é o ponto central da leaf. Exigir CPF em vez de aceitar telefone-só mantém a garantia de dedup no nível de força que D-020 já escolheu para esse propósito, em vez de degradar silenciosamente para um caso mais fraco só porque é mais rápido de preencher.
- **Consequências:** canon fechado; #86/#90 (Execução) implementam sem reabrir arquitetura; `jornadas-plataforma.md`/`screen-map.md` saem de rascunho para validado nesta jornada; nenhuma decisão fechada é reaberta por esta leaf — puramente consumidora de D-020 e da amarra de #189.

### D-064 — Jornada: pipeline e dashboard do corretor (2026-07-12) — **#191**

- **Status:** accepted
- **Contexto:** oitava leaf de Passo 5 (Jornadas, epic #176) grillada. `pages/corretor/dashboard.vue` e `leads/index.vue` predatam D-020 e carregam um enum de pipeline de 7 estágios (`new → contacted → visit_scheduled → visit_completed → negotiating → closed_won → closed_lost`) nunca formalizado em decisão nenhuma — vem das notas pré-arquitetura originais, não do modelo `registro` real (D-020 só nomeia `status` genericamente, sem enumerar).
- **Decisão:**
  - **`registro.status` definido pela primeira vez, como enum focado em negócio, não em visita:** `registrado → negociando → fechado_ganho | fechado_perdido` — não os 7 estágios do stub. Dois desses estágios (`visit_scheduled`, `visit_completed`) duplicavam estado que já pertence exclusivamente a `visit.status` (D-053); manter os dois em sincronia manual seria o mesmo risco de dessincronização que #188 e #190 já corrigiram em outros lugares nesta sessão (respectivamente: `cancelled` isolado de `declined`; dedup por CPF em vez de telefone).
  - **Progresso de visita é derivado via join, nunca duplicado:** o dashboard lê o(s) registro(s) de `visit` ligado(s) ao cliente/casa para mostrar contexto de visita — não guarda uma segunda cópia desse estado em `registro`.
  - **"Contatado" vira entrada de `historico`, não estágio formal:** um toque de contato é um evento a registrar (já suportado pelo `historico` append-only de D-020), não uma transição de estágio — evita a ambiguidade de "contatado uma vez" vs. "todo follow-up reseta o estágio".
  - **HubSpot pode mostrar um pipeline mais rico sem contradição:** D-020 já estabeleceu Supabase como autoridade de status comissionável e HubSpot como espelho a jusante — nada aqui impede a visualização do HubSpot ter mais estágios (visita agendada, contatado, etc.) para o fluxo de trabalho do corretor; é escolha de exibição do vendor, não muda o que o Supabase precisa guardar como verdade.
  - **Escopo por corretor:** dashboard e lista filtram por `registro.corretor_id` do corretor autenticado, com RLS como fronteira real (D-055, duas camadas) — corrige o filtro por `realtor_id` do stub contra a tabela legada.
  - **Auth corrigido:** `role = corretor` + `status = approved` (modelo de #189), não mais `realtors`.
  - **DoD:** docs only — D-064 + `templates/jornada-pipeline-dashboard-corretor.md` + nova capability OpenSpec `journey-corretor-pipeline` + `jornadas-plataforma.md` §4.4 e `screen-map.md` atualizados. Implementação real → Execução (#86, #90).
- **Rationale:** o enum de 7 estágios não é neutro — misturar o ciclo de vida da visita (já modelado à parte em D-053) com o ciclo de vida do negócio (o que `registro` de fato existe para rastrear) cria duas fontes de verdade para "o cliente visitou?", exatamente a classe de bug que #188/#190 já precisaram corrigir. Definir o enum agora, do jeito certo, evita herdar esse problema silenciosamente. Mover "contatado" para `historico` evita inventar uma máquina de estados para algo que é só um registro de evento.
- **Consequências:** canon fechado; nenhuma decisão fechada reaberta — `registro.status` nunca tinha enum formal, então defini-lo é completar um detalhe em aberto, não mudar um requirement existente; #86/#90 (Execução) implementam sem reabrir arquitetura; `jornadas-plataforma.md`/`screen-map.md` saem de rascunho para validado nesta jornada.

### D-065 — Jornada: operação diária do staff (2026-07-13) — **#193**

- **Status:** accepted
- **Contexto:** nona leaf de Passo 5 (Jornadas, epic #176) grillada. Tela operacional de staff (rascunho §5.1: visitas do dia, clientes recentes, entrada manual de lead) totalmente greenfield — nenhum código existia, mesma situação de #192 antes desta sessão.
- **Decisão:**
  - **Staff enxerga tudo, não escopado por corretor:** visitas do dia e clientes recentes mostram dados de todos os corretores/casas — diferente do pipeline de #191, escopado a `registro.corretor_id` do próprio corretor. Consequência direta da hierarquia RBAC de D-055 (staff/admin têm visão operacional mais ampla), não uma decisão de acesso nova.
  - **Entrada manual é nível `Contato`, não `Cliente` (diferente de #190):** a exigência de CPF de #190 protege atribuição de comissão quando um corretor está envolvido — sem corretor (`corretor_id` nulo, registro direto, já modelado em D-020), não há essa proteção a fazer, então exigir CPF aqui só adicionaria atrito sem benefício correspondente. Nível `Contato` (só WhatsApp), `fonte: staff-manual`, mesmo padrão kebab-case de `cta-whatsapp`/`form-site`/`portal-corretor`.
  - **Dedup reaproveita a reconciliação já existente de D-020, sem mecanismo novo:** número de WhatsApp que já existe como Contato/Cliente é reaproveitado, não duplicado — mesma regra que #185 e #190 já usam, só um terceiro ponto de entrada exercitando-a.
  - **Resumo de pendências que só linka, não reimplementa:** a tela mostra contagens da fila de aprovação de corretor/casa (#189) e da fila de exceção de verificação (#192), com link para cada tela já existente — mantém essas duas capabilities como donas únicas da própria lógica de aprovar/rejeitar.
  - **DoD:** docs only — D-065 + `templates/jornada-operacao-diaria-staff.md` + nova capability OpenSpec `journey-staff-daily-operations` + `jornadas-plataforma.md` §5.1 e `screen-map.md` atualizados. Implementação real → Execução (#86, #90, #81).
- **Rationale:** nenhum ponto de design nesta leaf exigiu uma decisão nova de fato — cada escolha (escopo staff-wide, nível Contato, reuso de dedup, resumo que só linka) é consequência direta de decisões já fechadas (D-020, D-053, D-055, #189/#191/#192), aplicadas corretamente pela primeira vez neste ponto de entrada.
- **Consequências:** canon fechado; nenhuma decisão fechada reaberta — leaf puramente consumidora; #86/#90/#81 (Execução) implementam sem reabrir arquitetura; `jornadas-plataforma.md`/`screen-map.md` saem de rascunho para validado nesta jornada.

### D-066 — Jornada: gestão de acesso Tuya (2026-07-13) — **#194**

- **Status:** accepted
- **Contexto:** décima leaf de Passo 5 (Jornadas, epic #176) grillada. O próprio corpo da issue já nomeava a resposta esperada: gestão de código de emergência Tuya (rotação, status do dispositivo) fica no Supabase Studio no v1, per D-052/D-056 — "não requer UI custom a menos que a escala mude essa decisão". Esta leaf é uma re-confirmação, não uma construção.
- **Decisão:**
  - **Teste de três partes de D-056 reaplicado — veredito inalterado:** (1) rotação de código não é workflow multi-etapa com efeitos colaterais (contraste com aprovação de corretor #189 ou revisão de verificação #192, que desbloqueiam acesso/ação); (2) não há renderização específica de domínio que Supabase Studio não faça; (3) D-056 já pesou e rejeitou o critério "segurança para staff não-técnico" para este caso exato — nada na escala do negócio mudou desde então.
  - **"Status do dispositivo" esclarecido, não expandido:** refere-se aos campos já existentes no registro do Supabase Studio (última rotação, em uso) — não um dashboard de monitoramento proativo. D-052 já decidiu que detecção de falha é reativa (alerta WhatsApp síncrono no momento do provisionamento), nunca pediu uma tela de status contínua.
  - **Nenhum conteúdo de decisão novo:** esta leaf formaliza a conclusão já fechada como um requirement de spec pela primeira vez (antes só existia em prosa de decisão) — satisfaz a exigência estrutural do OpenSpec de ter ao menos um delta, sem inventar escopo novo.
  - **DoD:** docs only — D-066 + nova capability OpenSpec `journey-tuya-access-management` (formalização, não decisão nova) + `jornadas-plataforma.md` e `screen-map.md` atualizados (re-confirmado, sem rota nova). Sem implementação — decisão já era "sem UI custom".
- **Rationale:** re-confirmar explicitamente, em vez de simplesmente pular a leaf, mantém o padrão de que toda sub-issue folha de #176 passa pelo ciclo completo (issue → OpenSpec change → close), mesmo quando a resposta correta é "nada muda" — a alternativa (fechar a issue sem registro algum) deixaria a re-validação implícita e não rastreável.
- **Consequências:** canon fechado; nenhuma decisão fechada reaberta nem nova decisão de arquitetura criada; D-052/D-056 permanecem a fonte de verdade; `jornadas-plataforma.md`/`screen-map.md` marcam esta tarefa como re-validada, sem mudança de abordagem.

### D-067 — Jornada: configuração de plataforma e papéis (2026-07-13) — **#195**

- **Status:** accepted
- **Contexto:** décima primeira e última leaf de Passo 5 (Jornadas, epic #176) grillada. Telas admin-only (rascunho §5.2: atribuição de papel, flags de plataforma, chaves API) predatam D-055/D-056/D-043. Diferente da maioria das leaves anteriores, o rascunho não só ficou desatualizado — contradiz diretamente duas decisões já fechadas.
- **Decisão:**
  - **Sem árvore `/admin/*` — tudo admin-only vive em `/staff/*`:** D-056 já rejeitou explicitamente uma segunda árvore de UI ("namespace único `/staff/*`... ações genuinamente Admin-only... checagem mais estrita por rota/ação dentro da mesma árvore, não uma segunda árvore"). Rascunho e `screen-map.md` nunca alcançaram essa decisão — corrigidos aqui: `/staff/usuarios`, `/staff/config`, `/staff/integracoes`.
  - **Tela de "chaves API" vira referência somente-leitura, não editor de secrets — contradição real com D-043:** D-043 já restringe escrita de secrets a "ForteGB tech" especificamente, não ao papel RBAC `admin` genericamente ("Sócios: sem acesso a secrets"). Como "Digital"/tech é fato organizacional distinto do papel `admin` (D-055 já faz essa distinção), um sócio-admin que não é tech (ex. Adilson) não deveria ter acesso a secrets sob D-043 — mas o rascunho implicava que qualquer admin poderia configurar chaves API. Corrigido: tela mostra só status de conexão/configuração; valores reais nunca aparecem no banco/UI da plataforma — edição real fica no Vercel, restrita a tech.
  - **"Ocultar casa" não precisa de tela na plataforma — Sanity Studio já resolve:** casas são conteúdo gerido no Sanity (D-015), que já tem workflow nativo de rascunho/despublicação. Construir uma flag redundante na própria UI admin repetiria o mesmo erro de over-building evitado para Tuya (#194) e para chaves API nesta mesma leaf.
  - **"Modo manutenção" é uma flag viva no Supabase, exceção deliberada ao padrão vendor-native acima:** diferente dos três casos anteriores desta leaf, não é um secret (sem racional de segurança de D-043) e seu propósito inteiro é resposta rápida a emergência. Uma env var Vercel exigiria redeploy e, por D-043, só poderia ser mudada por "ForteGB tech" — um sócio-admin percebendo um problema real não conseguiria agir sem antes contatar Ricardo/Felipe, anulando o propósito de um toggle de emergência. Flag viva, qualquer admin pode ligar/desligar na hora, sem deploy.
  - **Atribuição de papel é funcionalidade nova genuína, sem conflito:** admin convida por e-mail com papel pré-atribuído (`staff` ou `admin`) — diferente do onboarding de corretor (#189, autorregistro público + aprovação), staff/admin são pessoas conhecidas convidadas diretamente, sem fila de aprovação (o próprio convite do admin já é a aprovação). Aditivo a `rbac-role-model`, não modificação — mecanismo de convite nunca foi escopo dessa capability (só enum/hierarquia/enforcement).
  - **Fora de escopo:** "Relatórios agregados" e "Exceções de comissão / void de registro" do rascunho original — nenhum está no escopo declarado desta issue; permanecem lacunas sem leaf própria, não construídas aqui.
  - **DoD:** docs only — D-067 + `templates/jornada-configuracao-plataforma-papeis.md` + nova capability OpenSpec `journey-platform-admin-config` + `jornadas-plataforma.md` §5.2 e `screen-map.md` atualizados (rotas `/admin/*` → `/staff/*`, linha de ocultar-casa removida). Implementação real → Execução (#119, #72).
- **Rationale:** duas correções reais (rotas, chaves API) evitam que a plataforma construa exatamente o que duas decisões já fechadas proibiram — não são nuances, são contradições diretas que precisavam ser corrigidas antes da Execução herdar o rascunho errado. Deferir ocultar-casa ao Sanity mantém consistência com o padrão já estabelecido (Tuya, chaves API) de não reconstruir o que o vendor já resolve. Modo manutenção como exceção deliberada — não uma inconsistência — porque o padrão nunca foi "evite todo armazenamento in-app", foi "evite reconstruir o que um vendor já resolve", e nenhum vendor resolve "deixar qualquer admin tirar o site do ar instantaneamente".
- **Consequências:** canon fechado; nenhuma decisão fechada reaberta — leaf aplica testes/políticas já existentes (D-055/D-056/D-043) a telas concretas; #119/#72 (Execução) implementam sem reabrir arquitetura; `jornadas-plataforma.md`/`screen-map.md` saem de rascunho para validado nesta jornada; **última leaf de Passo 5 (epic #176) fechada.**

### D-068 — Corretor: CPF obrigatório no onboarding (2026-07-13) — **#196**

- **Status:** accepted
- **Contexto:** lacuna encontrada durante a exploração de #190 (registro de cliente e proteção de comissão), separada em issue própria por instrução do usuário em vez de reabrir #189 silenciosamente. D-062 (#189) exige WhatsApp e deixa CRECI opcional no perfil do corretor, mas nunca exigiu o CPF do próprio corretor.
- **Decisão:**
  - **CPF passa a ser campo obrigatório no cadastro, junto com WhatsApp — não diferido para aprovação ou primeiro pagamento:** o passo de preenchimento de perfil (D-062) já é o ponto em que o corretor está ativamente fornecendo dados; pedir CPF depois (ex. no primeiro pagamento de comissão) significaria cobrar um corretor já ativo por um dado que sempre foi conhecível no cadastro. Sem custo adicional no fluxo (um campo a mais no mesmo formulário).
  - **Sem novo estado em `corretor.status` nem mudança na fila de aprovação:** presença de CPF é validação de formulário (campo obrigatório, mesmo tratamento de WhatsApp), não um critério de aprovação adicional — staff continua aprovando/rejeitando a aplicação inteira como uma única decisão em `/staff/corretores`, inalterado desde D-062.
  - **Validação só de formato, sem verificação de terceiros:** mesmo rigor já aceito para o CPF do cliente em D-063 (#190) — verificar elegibilidade real de pagamento é tarefa do mecanismo de payout, ainda não desenhado.
  - **Reabre D-062 (`journey-corretor-onboarding`), registrado explicitamente** — mesmo tratamento que #187 deu a D-053 e #189 deu a `crm-source-of-truth`: uma correção pontual a uma decisão fechada, não uma decisão nova do zero.
  - **Fora de escopo:** o mecanismo de pagamento de comissão em si; CPF para qualquer outro papel (cliente já coberto por D-063, staff/admin fora de escopo).
...
### D-070 — Verificação de identidade: serviço externo, adapter e retenção (2026-07-20) — **#198**

- **Status:** accepted
- **Emenda:** reabre **D-053** (que rejeitou serviço externo de verificação: "`client-match` primário para os dois fluxos — sem split, sem KYC SaaS").
- **Contexto:** ao desenhar o estado de falha de câmera de [#198](https://github.com/fortegb/platform/issues/198) (leaf de design de Passo 6), ficou visível que o `client-match` com upload de arquivo não é evidência de presença: em desktop o atributo `capture` é ignorado, então a "selfie" pode ser qualquer imagem em disco. A verificação protege uma porta que destranca sozinha, sem ninguém presente. D-053 rejeitou serviço externo pesando-o contra uma biblioteca frontend gratuita — premissa que o volume real derruba.
- **Decisão:**
  - **Adotar serviço externo de verificação — Didit, plano gratuito.** 500 verificações completas por mês, permanentes, sem cartão e sem contrato. O pacote cobre os três problemas levantados no design: validação do documento, comparação selfie×documento, e **liveness passivo** (detecta foto-de-foto sem exigir piscar ou virar o rosto). Volume real da ForteGB: ~5 visitas/mês, meses inteiros sem nenhuma — o custo, que era o argumento de D-053, deixa de existir.
  - **Atrás de um adapter**, mesmo padrão do Tuya (D-052). O serviço é implementação, não arquitetura: trocá-lo não reabre o modelo de dados, as regras de retenção, nem nenhuma tela. É também a mitigação do risco real de plano gratuito — condição comercial que o fornecedor pode mudar.
  - **Harmonização de retenção — pegar o que é nosso e apagar a sessão.** A retenção do fornecedor é **por sessão**, não por artefato, então não é possível manter o documento e descartar a selfie lá dentro. Regra única:
    - **Aprovado:** baixar o documento para o nosso armazenamento, apagar a sessão. Selfie nunca é guardada — exatamente o que D-053 determina.
    - **Rejeitado ou em fila de exceção:** baixar documento **e** selfie, apagar a sessão. A purga de 30 dias da selfie (D-053) passa a rodar sobre o nosso armazenamento.
    - **Retenção do fornecedor no mínimo (1 mês)** — não é onde a regra vive; é rede de segurança para sessões que o nosso código falhou em limpar.
  - **As regras de retenção de D-053 seguem inalteradas** e passam a ser integralmente nossas: selfie efêmera (30 dias se rejeitada), documento 12 meses na janela do `Cliente`. O fornecedor guarda dados apenas nos minutos entre captura e resultado.
  - **Residência de dados — trade-off aceito e registrado.** Residência no Brasil é recurso de plano enterprise; no plano gratuito o processamento ocorre fora do país. Permitido pela LGPD com as salvaguardas devidas, mas é desvio do instinto "manter no Brasil". Mitigado pelo padrão acima: o fornecedor não retém nada.
  - **Marca própria no lançamento, plano gratuito durante o desenvolvimento.** No plano gratuito o visitante é redirecionado para uma página hospedada pelo fornecedor, com a marca dele. Isso é ruim exatamente no ponto mais frágil do fluxo — pedir foto de um documento é onde mais se abandona, e mandar a pessoa para um site estrangeiro desconhecido é o sinal errado ali. A opção com nossa marca (cores, logo, domínio) custa ~US$0,20 por sessão, ~R$6/mês no volume real. **Decisão:** usar o fluxo hospedado gratuito durante build e testes (sem gastar antes de validar) e **ligar a marca própria antes do lançamento público**. Sem isso, o custo do redirecionamento cai justamente sobre o primeiro visitante real.
  - **Captura ao vivo deixa de ser mecanismo nosso.** A decisão de exigir captura ao vivo (tomada no desenho de #198) é entregue pelo liveness passivo do fornecedor — sem desafio de piscar, o visitante só tira a foto.
  - **Caminho de exceção inalterado:** câmera indisponível, navegador embutido de Instagram/Facebook, ou falha do serviço caem no escape hatch de WhatsApp que D-053 já decidiu (`method: staff-review`), e o acesso continua saindo só por `provisionAccess()` depois de `visit.status = verified`.
  - **DoD:** este D-number + spike de confirmação prática ([#217](https://github.com/fortegb/platform/issues/217)) antes de qualquer build. Implementação real → Execução (#80).
- **Rationale:** o que mudou desde D-053 não foi o fornecedor, foi o ponto de observação — chegar ao desenho do detalhe expôs que upload de arquivo não prova presença. E a premissa de custo de D-053 não sobrevive a ~5 verificações/mês: o plano gratuito cobre 100x o volume. O adapter mantém a decisão reversível, e o padrão pegar-e-apagar deixa a retenção mais próxima do que D-053 pretendia do que a própria D-053 conseguia — nenhum arquivo em terceiro.
- **Consequências:** D-053 emendada apenas na escolha de mecanismo — modelo de dados (`Cliente`/`verification_attempt`/`visit`), hard gate, reuso de 12 meses, fila de exceção e regras de retenção seguem íntegros. #80 (Execução) implementa contra o adapter. [#216](https://github.com/fortegb/platform/issues/216) (mídia por WhatsApp) permanece necessária: o caminho de exceção continua trazendo arquivos por WhatsApp. Desenho das telas de #198 não muda — a captura passa a acontecer pelo SDK do fornecedor.

### D-071 — Remarcar visita: cancelar só na confirmação do novo horário (2026-07-20) — **#200**

- **Status:** accepted
- **Emenda:** ajusta a ordem definida em **D-061** ("botão Remarcar cancela a visita atual e entra no formulário de agendamento padrão, pré-preenchido").
- **Contexto:** ao desenhar a tela `/visita/gerenciar/[token]` ([#200](https://github.com/fortegb/platform/issues/200), quarta leaf de design de Passo 6, jornada #188/D-061), a regra de escopo 3 (todo ramo, feliz e triste) expôs um ramo que a prosa de D-061 criava sem querer: se "Remarcar" cancela a visita **ao ser clicado** e o visitante depois abandona o formulário de agendamento, ele fica **sem visita nenhuma** — perdeu o horário sem querer. A página é sem login, alcançada por um link de WhatsApp; abandono no meio do fluxo é comum exatamente com esse público.
- **Decisão:**
  - **A visita original permanece ativa** ao clicar Remarcar; o visitante entra no fluxo de agendamento padrão (#186) pré-preenchido, e a visita atual é **cancelada só quando o novo horário é confirmado** — não antes.
  - **O comportamento de cancelamento em si não muda:** na confirmação do novo horário, a visita antiga segue o mesmo caminho de D-061 (status `cancelled` + `revoke(credential)` se já havia código provisionado). Execução (#141) implementa as duas transições como um único commit no momento da confirmação.
  - **Intenção de D-061 preservada:** remarcar pelo fluxo normal (reuso de verificação, sem editar in-place — o "editar in-place" segue rejeitado). O que muda é só **o momento** do cancelamento, de "ao clicar" para "na confirmação".
  - **DoD:** este D-number + a tela desenhada (todos os estados), refletidos em `journey-post-visit-reengagement` (requirement "Reschedule cancels the original visit only on confirmation of the new slot"). Implementação real → Execução (#141, #81).
- **Rationale:** cancelar antes de haver um substituto confirmado transfere ao visitante o risco de uma falha de fluxo (abandono) que o produto pode evitar de graça — a visita só some quando há outra no lugar. O custo é nulo: o fluxo de agendamento já existe; só se adia uma transição de estado. Registrado como emenda (e não silenciosamente) pelo mesmo motivo de #187→D-053 e #196→D-062 — divergir da prosa de uma decisão fechada sem registro é o pior dos dois mundos.
- **Consequências:** D-061 emendada só na ordem do cancelamento; `revoke()`, o status `cancelled`, o alerta Telegram e o reuso de verificação seguem íntegros. Execução lê a ordem emendada, não a prosa original. Nenhuma outra tela afetada.

### D-072 — Corretor: notificação de status de conta por e-mail + WhatsApp, uniforme (2026-07-21) — **#201**

- **Status:** accepted
- **Emenda/estende:** **D-062** (que só previa notificação de **rejeição**, e só por **WhatsApp**); adiciona **e-mail** como canal, fora de **D-054** (WhatsApp/Telegram); reabre estreito o "e-mail futuro" de **D-020**.
- **Contexto:** ao revisar a jornada de onboarding do corretor ([#201](https://github.com/fortegb/platform/issues/201), leaf de design de Passo 6), o usuário decidiu que tanto **aprovação** quanto **rejeição** devem avisar o corretor. D-062 só decidira a rejeição (WhatsApp-only) e era silenciosa quanto à aprovação — e a copy da tela "em análise" já prometia aviso de aprovação por WhatsApp, sem decisão que a respaldasse. A pergunta do usuário na revisão expôs a lacuna.
- **Decisão:**
  - **Aprovação e rejeição notificam o corretor, ambas por e-mail e WhatsApp — política única, sem assimetria.** O corretor já cadastra um e-mail (auto-registro cria conta com e-mail + senha), e aviso de status de conta é mensagem **transacional** (sem opt-in, coerente com o espírito de D-054 para parte externa).
  - **E-mail passa a ser canal para esta classe de notificação de conta** — narrowing deliberado do "e-mail adiado" de D-020, restrito a notificações transacionais de conta do corretor. Não abre e-mail como canal de mensageria geral (marketing/visitas seguem como estão).
  - **Fora de D-054 por design:** D-054 cobre WhatsApp/Telegram; o e-mail aqui é aditivo a essa política, não uma reescrita dela — WhatsApp continua obrigatório para a parte externa; o e-mail é um segundo canal.
  - **DoD:** este D-number + requisito "Account-status results are notified by both e-mail and WhatsApp" na capability `journey-corretor-onboarding` + copy da tela de status ajustada ("avisa por e-mail e WhatsApp assim que sair o resultado"). Envio real → Execução (#86).
- **Rationale:** notificar nos dois canais dá redundância barata para um evento que o corretor está esperando (a decisão sobre a conta dele), e uniformizar aprovação/rejeição evita uma política torta ("aprovação em dois canais, rejeição em um"). O e-mail é gratuito e nativo (Supabase Auth), sem os gates de template do WhatsApp outbound — então adicioná-lo não custa build relevante. Registrado como decisão (e não só copy) porque toca três decisões fechadas (D-062, D-054, D-020).
- **Consequências:** `journey-corretor-onboarding` ganha o requisito de notificação; a implementação do envio fica em Execução (#86), junto do lado staff (upload = aprovação, #204). Nenhuma mudança em `rbac-role-model` ou no fluxo de associação por casa. Abre precedente: se outras notificações de conta quiserem e-mail, seguem este padrão; mensageria geral (D-054) permanece WhatsApp/Telegram até decisão própria.
