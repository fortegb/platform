...
const _inlineRuntimeConfig = {
  ...
  "public": {
    ...
    "supabaseAnonKey": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InBsYWNlaG9sZGVyIiwicm9sZSI6ImFub24iLCJpYXQiOjE2NDU5OTk5OTksImV4cCI6MTk2MTU3NTk5OX0.placeholder",
    "whatsappNumber": "5511999999999",
    "supabase": {
      "url": "https://placeholder.supabase.co",
      "key": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InBsYWNlaG9sZGVyIiwicm9sZSI6ImFub24iLCJpYXQiOjE2NDU5OTk5OTksImV4cCI6MTk2MTU3NTk5OX0.placeholder",
      "redirect": false,
      "redirectOptions": {
        "login": "/login",
        "callback": "/confirm",
  ...
  "supabaseUrl": "https://placeholder.supabase.co",
  "supabaseServiceKey": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InBsYWNlaG9sZGVyIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTY0NTk5OTk5OSwiZXhwIjoxOTYxNTc1OTk5fQ.placeholder",
  "hubspotApiKey": "",
  "tuyaAccessId": "",
  "tuyaAccessSecret": "",
  "whatsappApiKey": "",
  ...
  "supabase": {
    "serviceKey": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InBsYWNlaG9sZGVyIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTY0NTk5OTk5OSwiZXhwIjoxOTYxNTc1OTk5fQ.placeholder",
    "secretKey": ""
  }
};
const envOptions = {
...
const contact_post = defineEventHandler(async (event) => {
  ...
  if (!body.name || !body.email || !body.phone || !body.message) {
    throw createError({
      statusCode: 400,
      message: "Campos obrigat\xF3rios: name, email, phone, message"
    });
  }
  return {
    success: true,
...
const casas_get = defineEventHandler(async (event) => {
  const config = useRuntimeConfig();
  const supabase = createClient(config.supabaseUrl, config.supabaseServiceKey);
  const { data: houses, error } = await supabase.from("houses").select("*").in("status", ["disponivel", "em-construcao", "reservado"]).order("created_at", { ascending: false });
  if (error) {
    throw createError({
      statusCode: 500,
...
const casas_get$1 = /*#__PURE__*/Object.freeze(/*#__PURE__*/Object.defineProperty({
  __proto__: null,
  default: casas_get
}, Symbol.toStringTag, { value: 'Module' }));
...
const dashboard_get = defineEventHandler(async (event) => {
  ...
  const { data: realtor, error: realtorError } = await supabase.from("realtors").select("*").eq("user_id", user.id).single();
  ...
  const { data: allLeads, error: leadsError } = await supabase.from("leads").select("status").eq("realtor_id", realtor.id);
  if (leadsError) {
    throw createError({
      statusCode: 500,
      message: "Erro ao buscar estat\xEDsticas: " + leadsError.message
    });
  }
  const stats = {
    totalLeads: allLeads.length,
    activeLeads: allLeads.filter(
      (l) => ["new", "contacted", "visit_scheduled", "visit_completed", "negotiating"].includes(l.status)
    ).length,
    negotiating: allLeads.filter((l) => l.status === "negotiating").length
  };
  const { data: recentLeads, error: recentError } = await supabase.from("leads").select(`
      *,
      houses (
        id,
        title
      )
    `).eq("realtor_id", realtor.id).order("created_at", { ascending: false }).limit(5);
  const formattedRecentLeads = (recentLeads || []).map((lead) => {
    var _a;
    return {
      ...lead,
      house_title: ((_a = lead.houses) == null ? void 0 : _a.title) || null
    };
  });
  return {
    stats,
...
const leads_get = defineEventHandler(async (event) => {
  ...
  const { data: leads, error: leadsError } = await supabase.from("leads").select(`
    ...
    `).eq("realtor_id", realtor.id).order("created_at", { ascending: false });
  if (leadsError) {
    throw createError({
      statusCode: 500,
...
const leads_post = defineEventHandler(async (event) => {
  ...
  if (!body.name || !body.phone || !body.houseId) {
    throw createError({
      statusCode: 400,
      message: "Campos obrigat\xF3rios: name, phone, houseId"
    });
  }
  const { data: existingLead, error: checkError } = await supabase.from("leads").select("*").eq("phone", body.phone).eq("house_id", body.houseId).single();
  if (existingLead) {
    if (existingLead.realtor_id === realtor.id) {
      throw createError({
        statusCode: 400,
        message: "Este lead j\xE1 foi registrado por voc\xEA."
      });
    } else {
      throw createError({
        statusCode: 400,
        message: "Este lead j\xE1 foi registrado por outro corretor. A comiss\xE3o ser\xE1 creditada ao primeiro corretor que registrou."
      });
    }
  }
  ...
  return {
    success: true,
    lead,
    message: "Lead registrado com sucesso! Voc\xEA ser\xE1 creditado pela comiss\xE3o se este lead fechar neg\xF3cio."
  };
});
async function syncLeadToHubSpot(data) {
...
const delete_post = defineEventHandler(async (event) => {
  ...
  if (!body.email && !body.phone) {
    throw createError({
      statusCode: 400,
      message: "\xC9 necess\xE1rio fornecer e-mail ou telefone para identificar os dados"
    });
  }
  let userData = [];
  if (body.email) {
    const { data: leadsByEmail } = await supabase.from("leads").select("*").eq("email", body.email);
    if (leadsByEmail) userData.push(...leadsByEmail);
  }
  if (body.phone) {
    const { data: leadsByPhone } = await supabase.from("leads").select("*").eq("phone", body.phone);
    if (leadsByPhone) userData.push(...leadsByPhone);
  }
  let visits = [];
  if (body.email || body.phone) {
    const { data: visitsData } = await supabase.from("visits").select("*").or(`visitor_phone.eq.${body.phone},visitor_email.eq.${body.email}`);
  ...
  return {
    success: true,
    message: "Solicita\xE7\xE3o de exclus\xE3o processada. Dados sens\xEDveis foram removidos ou anonimizados conforme nossa pol\xEDtica de reten\xE7\xE3o."
  };
});
...
const instant_post = defineEventHandler(async (event) => {
  const body = await readBody(event);
  const config = useRuntimeConfig();
  if (!body.qrCode || !body.verificationData) {
    throw createError({
      statusCode: 400,
      message: "Campos obrigat\xF3rios: qrCode, verificationData"
    });
  }
  if (!body.verificationData.verified) {
    throw createError({
      statusCode: 403,
      message: "Verifica\xE7\xE3o de identidade falhou"
    });
  }
  const supabase = createClient(config.supabaseUrl, config.supabaseServiceKey);
  const { data: house, error: houseError } = await supabase.from("houses").select("*").eq("qr_code", body.qrCode).single();
  if (houseError || !house) {
    throw createError({
      statusCode: 404,
      message: "C\xF3digo QR inv\xE1lido"
    });
  }
  ...
  try {
    await programSmartLock$1(house.id, password, now, expiresAt);
  } catch (err) {
  ...
  if (body.verificationData.phone) {
    try {
      await sendWhatsAppMessage$1(
        ...
        `Sua senha de acesso: ${password}. Esta senha \xE9 v\xE1lida por 2 horas.`
      );
    } catch (err) {
...
async function programSmartLock$1(houseId, password, startDate, endDate) {
...
async function createHubSpotLead$1(data) {
  console.log("HubSpot lead would be created:", data);
}

const instant_post$1 = /*#__PURE__*/Object.freeze(/*#__PURE__*/Object.defineProperty({
  __proto__: null,
  default: instant_post
}, Symbol.toStringTag, { value: 'Module' }));
...
const _code__get = defineEventHandler(async (event) => {
  ...
  if (!code) {
    throw createError({
      statusCode: 400,
      message: "C\xF3digo QR n\xE3o fornecido"
    });
  }
  const supabase = createClient(config.supabaseUrl, config.supabaseServiceKey);
  const { data: house, error } = await supabase.from("houses").select("*").eq("qr_code", code).single();
  if (error || !house) {
    return {
      success: false,
      message: "C\xF3digo QR inv\xE1lido"
...
const schedule_post = defineEventHandler(async (event) => {
  ...
  if (!body.houseId || !body.name || !body.phone || !body.date || !body.time || !body.verificationData) {
    throw createError({
      statusCode: 400,
      message: "Campos obrigat\xF3rios: houseId, name, phone, date, time, verificationData"
    });
  }
  if (!body.verificationData.verified) {
    throw createError({
  ...
  const visitDate = /* @__PURE__ */ new Date(`${body.date}T${body.time}`);
  const expiresAt = new Date(visitDate.getTime() + 4 * 60 * 60 * 1e3);
  const supabase = createClient(config.supabaseUrl, config.supabaseServiceKey);
  const { data: visit, error: dbError } = await supabase.from("visits").insert({
    house_id: body.houseId,
  ...
  try {
    await createCalendarEvent({
      ...
      start: visitDate,
      end: new Date(visitDate.getTime() + 60 * 60 * 1e3)
      // 1 hora
    });
  } catch (err) {
    console.error("Erro ao criar evento no calend\xE1rio:", err);
  }
  try {
  ...
  try {
    await sendWhatsAppMessage(
      body.phone,
      `Ol\xE1 ${body.name}! Sua visita foi agendada para ${formatDate(visitDate)} \xE0s ${body.time}. Sua senha de acesso: ${password}. Esta senha \xE9 v\xE1lida por 4 horas a partir do hor\xE1rio agendado.`
    );
  } catch (err) {
    console.error("Erro ao enviar WhatsApp:", err);
  }
...
async function programSmartLock(houseId, password, startDate, endDate) {
  console.log("Smart lock would be programmed:", { houseId, password, startDate, endDate });
}
...
const verify_post = defineEventHandler(async (event) => {
  ...
  if (!body.selfie || !body.document) {
    throw createError({
      statusCode: 400,
      message: "Campos obrigat\xF3rios: selfie, document"
    });
  }
  const verificationScore = 0.85;
  const verified = verificationScore > 0.7;
  return {
    success: true,
    verified,
