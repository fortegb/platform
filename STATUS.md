...
→ PRÓXIMO: **[#199 — Design e tokenização: Visita QR](https://github.com/fortegb/platform/issues/199)** — rota `/visita/qr/[code]`. Base de jornada: [#187](https://github.com/fortegb/platform/issues/187)/D-059. Herda a fundação de #197 e os padrões de tela de #198 (handoff de verificação, tela de resultado). Ver `design-system-fluxo.md` para a sequência das 11 leaves (#197–#207) + 4 varreduras (#208–#211); #197 entrega a fundação de tokens que as demais herdam. **`v0 — Definição` (gate G2): 16 abertos**, agora só trabalho nosso — nada esperando asset externo (higiene de 2026-07-19). G2 ainda fecha só depois do Passo 7 (Versionamento).
